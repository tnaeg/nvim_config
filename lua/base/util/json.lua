...
function M.save()
	...
	local path = vim.fn.stdpath("config") .. "/base.json"
...
function M.migrate()
	Util.info("Migrating `base.json` to version `" .. Config.json.version .. "`")
	...
	if not json.data.version then
		...
		json.data.extras = vim.tbl_map(function(extra)
			return "base.plugins.extras." .. extra
		end, json.data.extras or {})
	elseif json.data.version == 1 then
		json.data.extras = vim.tbl_map(function(extra)
			-- replace double extras module name
			return extra:gsub("^base%.plugins%.extras%.base%.plugins%.extras%.", "base.plugins.extras.")
		end, json.data.extras or {})
	end
