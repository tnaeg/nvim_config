...
function M.extra_idx(name)
	...
	for i, extra in ipairs(Config.spec.modules) do
		if extra == "base.plugins.extras." .. name then
			return i
		end
	end
end
...
function M.fix_imports()
	Plugin.Spec.import = Util.inject.args(Plugin.Spec.import, function(_, spec)
		...
		if dep then
			...
			Util.warn(dep, { title = "base", once = true, stacktrace = true, stacklevel = 6 })
			return false
		end
	end)
end
