...
function M.format(opts)
	...
	if not done and opts and opts.force then
		Util.warn("No formatter available", { title = "Config" })
	end
end
...
function M.health()
	...
	local has_extra = vim.tbl_contains(Config.spec.modules, "base.plugins.extras.lsp.none-ls")
	if has_plugin and not has_extra then
		Util.warn({
			"`conform.nvim` and `nvim-lint` are now the default formatters and linters in base.",
			"",
			"You can use those plugins together with `none-ls.nvim`,",
			"but you need to enable the `base.plugins.extras.lsp.none-ls` extra,",
