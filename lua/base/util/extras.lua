...
---@class base.util.extras
local M = {}

---@type LazyExtraSource[]
M.sources = {
	{ name = "base", desc = "base extras", module = "base.plugins.extras" },
	{ name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("base.extras")
---@type string[]
M.state = nil
...
function X.new()
	...
	self.float = Float.new({ title = "base Extras" })
		...
		if extra.row == pos[1] then
			...
			Util.info(
				"`"
					...
					.. "\nPlease restart base to apply the changes.",
				{ title = "LazyExtras" }
			)
			self:update()
			return
...
function X:render()
	self.text:nl():nl():append("base Extras", "LazyH1"):nl():nl()
	self.text
		:append("This is a list of all enabled/disabled base extras.", "LazyComment")
		:nl()
...
function X:extra(extra)
	...
	if extra.source.name ~= "base" then
