...
function M.setup(_, opts)
	...
	for _, key in ipairs({ "format_on_save", "format_after_save" }) do
		if opts[key] then
			Util.warn(
				("Don't set `opts.%s` for `conform.nvim`.\n**base** will use the conform formatter automatically"):format(
					key
				)
			)
			opts[key] = nil
		...
		opts = function()
			...
			if plugin.config ~= M.setup then
				Util.error({
					...
					"This will break **base** formatting.\n",
					"Please refer to the docs at https://www.base.org/plugins/formatting",
				}, { title = "base" })
			end
			---@class ConformOpts
			return {
				-- base will use these options when formatting with the conform.nvim formatter
				format = {
					timeout_ms = 1000,
				},
				formatters_by_ft = {
				...
				-- base will merge the options you set here with builtin formatters.
